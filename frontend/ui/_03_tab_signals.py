...
def _render_pairing_section(dataset_id: str, profile: Dict[str, Any]):
    """
    Stage 1: Generate column pairings and let the user select which ones to visualize.
    Shown in the 'After cleaning' tab only.
    """
    ...
    if st.button("Generate Column Pairings", type="primary", key=f"btn_pairings_{dataset_id}"):
        with st.spinner("Running Pairing Agent (Stage 1)…"):
            try:
                ...
                # Clear any stale plot selections when pairings are regenerated
    ...
    # Build display table with checkboxes
    pairing_rows = []
...
def render_tab_signals(cleaning_report: dict):
    ...
    with tab_post:
        ...
        # Pairing section — only when we have a post profile and a dataset_id
